...
class Solar(object):
    def __init__(self, year, month, day):
        ...
        try:
            ...
            raise DateNotExist(
                "Solar({}, {}, {}) doesn't exist".format(year, month, day))

    def __eq__(self, other):
        _other = Converter.Lunar2Solar(
            other) if isinstance(other, Lunar) else other
    ...
    def __init__(self, year, month, day, isleap=False, check=True):
        ...
        if check:
            ...
            if self != Converter.Solar2Lunar(_solar):
                raise DateNotExist("Lunar({}, {}, {}, {}) doesn't exist".format(
                    year, month, day, isleap))

    def __eq__(self, other):
        _other = Converter.Solar2Lunar(
            other) if isinstance(other, Solar) else other
...
class Converter(object):
    ...
    def Lunar2Solar(lunar):
        days = Converter.lunar_month_days[lunar.year -
                                          Converter.lunar_month_days[0]]
    ...
    def Solar2Lunar(solar):
        ...
        m = GetBitInt(solar11, 4, 5)
        d = GetBitInt(solar11, 5, 0)
        offset = SolarToInt(solar.year, solar.month,
        ...
        lunarD = int(offset)
        isleap = ((leap != 0) and (lunarM == leap + 1))
        if leap != 0 and lunarM > leap:
            lunarM -= 1

        return Lunar(lunarY, lunarM, lunarD, isleap, check=False)
